COPYRIGHT = "Copyright 2012-2014 Chris Patterson, All rights reserved."
...
props = {
  ...
  :keyfile => File.expand_path("Taskular.snk")
}
...
task :compile => [:versioning, :global_version, :build] do
	copyOutputFiles File.join(props[:src], "Taskular/bin/Release"), "Taskular.{dll,pdb,xml}", File.join(props[:output], 'net-4.5')
end
...
nunit :tests => [:compile] do |nunit|
          ...
          nunit.assemblies = FileList[File.join(props[:src], "Taskular.Tests/bin/Release", "Taskular.Tests.dll")]
end
...
zip :zip_output => [:versioning] do |zip|
	...
	zip.output_file = "Taskular-#{NUGET_VERSION}.zip"
...
msbuild :nuget_restore do |msb|
  ...
  msb.solution = 'src/Taskular.Tests/Taskular.Tests.csproj'
end

desc "Builds the nuget package"
task :nuget => [:versioning, :create_nuspec] do
	sh "src/.nuget/nuget.exe pack #{props[:artifacts]}/Taskular.nuspec /Symbols /OutputDirectory #{props[:artifacts]}"
end
...
nuspec :_nuspec do |nuspec|
  ...
  nuspec.description = 'Taskular, Composition Extensions for the Task Parallel Library'
  nuspec.title = 'Taskular'
  nuspec.projectUrl = 'http://github.com/phatboyg/Taskular'
  nuspec.language = "en-US"
  nuspec.licenseUrl = "http://www.apache.org/licenses/LICENSE-2.0"
  nuspec.requireLicenseAcceptance = "false"
  nuspec.output_file = File.join(props[:artifacts], 'Taskular.nuspec')
  add_files props[:output], 'Taskular.{dll,pdb,xml}', nuspec
  nuspec.file(File.join(props[:src], "Taskular\\**\\*.cs").gsub("/","\\"), "src")
end
